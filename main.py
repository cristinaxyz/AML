...
def main():
    ...
    st.markdown("<h3 style='color:pink'>Upload a '.h5' MRI scan slice. Returns a '.png' file with the segmentation mask with 3 regions of the tumor identified. Our model performs segmentation on MRI scan slices. We trained (model info). It was trained on BraTS2020 dataset, which included MRI scan slices with segmentations performed manually by neuro-radiologists. </h3>", unsafe_allow_html=True)
    ...
    if uploading != None:
        ...
        if st.button("Run Segmentation"):
            ...
            if response.status_code == 200:
                ...
                st.download_button(label = "Download '.png'!", data=response.content, file_name="segmentation.png")
            else:
